...
2. **HEIC demand** → the first server-side path. jSquash runs in most edge runtimes,
   so the codecs would not change — but GitHub Pages cannot host one, so this is also
   the point at which hosting moves again (ADR-0008).
...
process, no runtime. Hosting is GitHub Pages, published by
`.github/workflows/pages.yml` on a merge to `main`. The reasoning, and what moving off
a header-capable host cost, is in [ADR-0008](adr/0008-github-pages-hosting.md).

Three consequences are worth knowing before touching any of this:

**The security policy travels in the document, not in headers.** GitHub Pages sets no
response headers and offers no way to add any, so the CSP and referrer policy are
emitted as `<meta>` tags, built by `metaContentSecurityPolicy()` in
`config/security.ts` and injected into every page by `scripts/postbuild.mjs`.

They are injected rather than rendered from the root layout because a meta policy
covers only what is fetched after the browser parses it, and Next hoists its own
stylesheet and script tags above anything a layout renders into `<head>` — a
layout-rendered policy sits below the scripts it should govern and silently misses
them. The script checks its own output for exactly that and fails the build. That function strips the
directives a meta tag cannot carry — `frame-ancestors` among them — rather than
emitting protection the browser will silently drop. The four headers with no meta
equivalent are gone; `headerOnlyProtections` records what each cost. `securityHeaders`
is still the complete list and still the single source of truth, so a move back to a
header-capable host restores all six with no code change.

`headers()` in `next.config.ts` applies only to `next dev`. It is kept because losing
the CSP in development is how a violation reaches production unnoticed.

**The site is served from `/img-hub`, not a root.** A Pages project site lives at
`/<repo>`. `basePath` comes from `site.basePath`, derived in `config/site.ts` from
`NEXT_PUBLIC_SITE_URL` — one input, so the prefix on links and assets cannot drift
from the prefix on canonical URLs. Application code never needs it: `next/link` and
the bundler apply it. CI builds and serves under the subpath for exactly this reason,
since a root-served test cannot see a base-path mistake and such a mistake breaks
every page at once.

**`next start` does not work with a static export.** `pnpm start` runs
`scripts/serve.mjs` instead, a static server that honours the base path and
deliberately sets no security headers, matching the host. That is what the smoke
suite, the SEO audit and the vitals run point at, so those runs exercise the policy
production actually serves. It is a test harness, not production: no caching, no
compression.
